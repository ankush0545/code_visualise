...
DEFAULT_CACHE_DIR = os.path.expanduser("~/repo_cache")
...
def _clone_repo_into(url: str, dest_dir: str, branch: str | None = None) -> None:
    """Clone *url* directly into *dest_dir* (must not already exist).

    Used for the persistent-cache path: unlike _clone_repo(), which clones
    into a throwaway temp dir the caller is expected to delete, this leaves
    the checkout in place under --cache-dir so later pipeline stages (e.g.
    build_dataset.py) can reuse the same source without re-cloning."""
    os.makedirs(os.path.dirname(dest_dir) or ".", exist_ok=True)
    cmd = ["git", "clone", "--depth", "1"]
    if branch:
        cmd += ["--branch", branch]
    cmd += [url, dest_dir]
    print(f"  Cloning {url!r} into cache → {dest_dir!r} …")
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
...
def load_sources(args: list[str] | None = None) -> list[dict]:
    ...
    *args* defaults to sys.argv[1:].
    ...
    # ── Clone (remote URL) ───────────────────────────────────────────────
    # If a cache_dir was given, clone straight into it so the checkout
    # persists for later pipeline stages (e.g. build_dataset.py rereading
    # source content) instead of vanishing the moment this run's cleanup
    # happens. Only genuinely-temporary clones (no cache_dir configured)
    # go through the old throwaway-temp-dir path.
    persisted_to_cache = bool(cache_dir)
    if persisted_to_cache:
        repo_name = _repo_name_from_url(url)
        clone_dir = os.path.join(cache_dir, repo_name)
        _clone_repo_into(url, clone_dir, branch)
    else:
    ...
    try:
        ...
        # clean it up right away here. A cache-dir clone gets the same
        # treatment on error -- a half-cloned repo isn't worth keeping.
        shutil.rmtree(clone_dir, ignore_errors=True)
        print(f"  Deleted clone (after error) → {clone_dir}")
        raise

    if persisted_to_cache:
        # Left in place under cache_dir permanently -- nothing to clean up,
        # this behaves like an already-cached/local repo from here on.
        return sources
